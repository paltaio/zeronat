...
/// terminates the session. P1 builds it; the FSM does not require it.
    ...
    pub fn on_frame(&mut self, frame: &[u8]) -> Action {
        ...
        match self.state {
            State::PadiSent => {
                ...
                // First valid PADO wins; later PADOs are ignored (P5 picks).
...
mod tests {
    ...
    // Section 3.1: PADI we build (32 bytes).
    const PADI: &[u8] = &[
    ...
    // Section 3.2: PADR we build (32 bytes).
    const PADR: &[u8] = &[
    ...
    // Section 3.3: PADO the AC sends (45 bytes).
    const PADO: &[u8] = &[
    ...
    // Section 3.4: PADS the AC sends (32 bytes).
    const PADS: &[u8] = &[
        0xfe, 0xee, 0x13, 0xac, 0xc4, 0x74, 0x70, 0x7b, 0xe8, 0x74, 0x22, 0x17, 0x88, 0x63, 0x11,
